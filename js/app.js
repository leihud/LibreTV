...
function resetSearchArea() {
    ...
    try {
        ...
        // 更新页面标题
        document.title = `蚕宝宝TV - 免费在线视频搜索与观看平台`;
    } catch (e) {
                ...
                <div class="card-hover bg-[#111] rounded-lg overflow-hidden cursor-pointer transition-all hover:scale-[1.02] h-full shadow-sm hover:shadow-md" 
                    ...
                    <div class="flex h-full">
                        <div class="relative flex-shrink-0 search-card-img-container">
                            <img src="${coverUrl || generateDefaultCover(safeName)}" alt="${safeName}" 
                                 class="h-full w-full object-cover transition-transform hover:scale-110" 
                                 onerror="this.onerror=null; this.src=generateDefaultCover('${safeName}'); this.classList.add('object-contain');" 
                                 loading="lazy">
...
async function importConfigFromUrl() {
    ...
    document.getElementById('confirmUrlImport').addEventListener('click', async () => {
        ...
        try {
            ...
            if (config.name !== '蚕宝宝TV-Settings') throw '配置文件格式不正确';
...
async function exportConfig() {
    ...
    config['name'] = '蚕宝宝TV-Settings';  // 配置文件名，用于校验
    ...
    // 将配置数据保存为 JSON 文件
    saveStringAsFile(JSON.stringify(config), '蚕宝宝TV-Settings_' + times + '.json');
}
